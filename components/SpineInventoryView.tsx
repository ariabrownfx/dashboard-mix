    ...
    <div className="fixed inset-0 z-[60] bg-background-light dark:bg-background-dark flex flex-col animate-in slide-in-from-right duration-300">
      ...
      <div className="flex-1 overflow-y-auto pb-10">
        ...
        <section className="p-4 space-y-4">
          ...
          {/* Total Inventory Worth (Stock Worth + Estimated Profit) */}
          <div className="bg-emerald-600 dark:bg-emerald-500 rounded-3xl p-6 text-white shadow-lg relative overflow-hidden animate-in fade-in zoom-in-95 duration-500">
             <div className="absolute top-0 right-0 p-4 opacity-20 transform translate-x-4 -translate-y-4">
                <Icon name="payments" className="text-[120px]" />
             </div>
             <div className="relative z-10">
                <p className="text-[10px] font-black uppercase tracking-[0.2em] text-emerald-100 mb-1">Total Inventory Value</p>
                <h2 className="text-4xl font-black mb-2 tracking-tighter">₦{stats.expectedRevenue.toLocaleString()}</h2>
                <div className="flex items-center gap-2 py-2 px-3 bg-white/10 rounded-xl backdrop-blur-md border border-white/5 w-fit">
                   <Icon name="info" className="text-xs text-emerald-100" />
                   <p className="text-[9px] font-bold text-emerald-50 tracking-wide">
                     ₦{stats.stockWorth.toLocaleString()} (Cost) + ₦{stats.potentialProfit.toLocaleString()} (Profit)
                   </p>
                </div>
             </div>
          </div>
