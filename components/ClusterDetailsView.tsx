...
import { Collection, Cluster } from '../types';
...
export const ClusterDetailsView: React.FC<ClusterDetailsViewProps> = ({ collection, onBack }) => {
  const collectionClusters = CLUSTERS.filter(c => c.collectionId === collection.id);
        ...
        {activeTab === 'overview' ? (
            <div className="py-6 flex flex-col gap-4 animate-in fade-in slide-in-from-bottom-2 duration-300">
                ...
                <div className="p-4 rounded-xl bg-white dark:bg-slate-800/50 border border-slate-100 dark:border-slate-700/50 shadow-sm">
                    <p className="text-sm text-slate-500 dark:text-slate-400 mb-1">Max Return</p>
                ...
                <div className="space-y-4">
                    <h3 className="text-lg font-bold text-slate-800 dark:text-white">Available Clusters</h3>
                    <p className="text-sm text-slate-500 dark:text-slate-400 -mt-3 mb-2">Choose an investment cycle that fits your goals.</p>
                    
                    <div className="flex flex-col gap-4">
                        {collectionClusters.map((cluster) => (
                        <div 
                            key={cluster.id}
                            className="rounded-xl p-4 bg-white dark:bg-slate-800 border border-slate-100 dark:border-slate-700 shadow-sm hover:shadow-md transition-shadow"
                        >
                            ...
                            <div className="text-right">
                                ...
                                <span className={`text-[10px] uppercase font-bold px-2 py-0.5 rounded-full ${cluster.status === 'Open' ? 'bg-emerald-100 text-emerald-700 dark:bg-emerald-500/20 dark:text-emerald-400' : 'bg-amber-100 text-amber-700 dark:bg-amber-500/20 dark:text-amber-400'}`}>
                                {cluster.status}
                                </span>
                            </div>
                            </div>
                            ...
                            <div className="flex justify-between text-xs font-medium">
                                ...
                                <span className="text-slate-800 dark:text-white">{cluster.fundingProgress}%</span>
                            </div>
                            ...
                            <div className="grid grid-cols-2 gap-3 mt-4">
                                ...
                                <button 
                                    onClick={() => setSelectedCluster(cluster)}
                                    className="py-2.5 px-4 rounded-lg font-bold text-sm bg-primary text-white shadow-sm hover:opacity-90 transition-opacity"
                                >
                                    Invest Now
                                </button>
                            </div>
            ...
            <div className="py-6 flex flex-col gap-6 animate-in fade-in slide-in-from-right-2 duration-300">
                {collectionUpdates.length > 0 ? (
                    collectionUpdates.map((update, idx) => (
                ...
                {collectionUpdates.length > 0 && (
                    <div className="text-center">
                        <p className="text-xs text-slate-400">End of updates</p>
                    </div>
                )}
            </div>
        )}
