...
export const SecondaryMarketView: React.FC<SecondaryMarketViewProps> = ({ onBack, userTier }) => {
  ...
  const hasAccess = true; // userTier === 'Gold' || userTier === 'Platinum';

  // Mock Repayment rates (since it's not in the base type)
  const getRepaymentRate = (id: string) => {
      // Deterministic pseudo-random based on string char codes
      const sum = id.split('').reduce((acc, char) => acc + char.charCodeAt(0), 0);
      return 97 + (sum % 30) / 10; // Between 97.0 and 99.9
  };
    ...
    <div className="fixed inset-0 z-[60] bg-background-light dark:bg-background-dark flex flex-col animate-in slide-in-from-right duration-300">
      ...
      <header className="flex items-center gap-4 p-4 sticky top-0 bg-background-light/95 dark:bg-background-dark/95 backdrop-blur-md z-10 border-b border-slate-200 dark:border-slate-800">
        ...
        <div className="flex-1">
            ...
            <p className="text-xs text-amber-500 font-bold uppercase tracking-wider">Premium Access</p>
        </div>
      ...
      <div className="flex-1 overflow-y-auto pb-20">
        ...
        {/* Regions Tabs */}
        <div className="px-4 py-4 overflow-x-auto no-scrollbar bg-white dark:bg-slate-900 border-b border-slate-100 dark:border-slate-800 sticky top-[73px] z-10">
        ...
        <div className="px-4 py-4 space-y-4">
            {filteredOpportunities.map((market) => (
                <div key={market.id} className="bg-white dark:bg-slate-800 rounded-xl border border-slate-100 dark:border-slate-700 shadow-sm overflow-hidden hover:shadow-md transition-shadow group">
                    <div className="p-4 flex justify-between items-start">
                        <div className="flex items-center gap-4">
                             <div className="size-12 rounded-xl bg-slate-50 dark:bg-slate-700 flex items-center justify-center text-slate-500 dark:text-slate-300 group-hover:bg-blue-50 dark:group-hover:bg-blue-500/20 group-hover:text-blue-500 transition-colors">
                                <Icon name={market.iconName} className="text-2xl" />
                             </div>
                             <div>
                                 <h3 className="font-bold text-slate-800 dark:text-white text-base">{market.name}</h3>
                                 <div className="flex items-center gap-1.5 mt-0.5">
                                    <Icon name="public" className="text-xs text-slate-400" />
                                    <p className="text-xs text-slate-500 dark:text-slate-400">{market.location}, {market.country}</p>
                                 </div>
                             </div>
                        </div>
                    </div>
                    ...
                    <div className="px-4 pb-4 grid grid-cols-3 gap-2">
                        <div className="bg-slate-50 dark:bg-slate-700/50 p-2 rounded-lg">
                            <p className="text-[10px] text-slate-500 dark:text-slate-400 uppercase font-bold mb-0.5">Target ROI</p>
                            <p className="text-lg font-bold text-emerald-500">{market.maxReturn}%</p>
                        </div>
                        <div className="bg-slate-50 dark:bg-slate-700/50 p-2 rounded-lg">
                            <p className="text-[10px] text-slate-500 dark:text-slate-400 uppercase font-bold mb-0.5">Repayment Rate</p>
                            <p className="text-lg font-bold text-slate-800 dark:text-white">{getRepaymentRate(market.id).toFixed(1)}%</p>
                        </div>
                        <div className="bg-slate-50 dark:bg-slate-700/50 p-2 rounded-lg">
                            <p className="text-[10px] text-slate-500 dark:text-slate-400 uppercase font-bold mb-0.5">Risk Level</p>
                            <p className={`text-lg font-bold ${market.riskLevel === 'Low' ? 'text-emerald-500' : market.riskLevel === 'Medium' ? 'text-amber-500' : 'text-rose-500'}`}>
                                {market.riskLevel}
                            </p>
                        </div>
                    </div>
                    ...
                    <div className="px-4 pb-4">
                         ...
                         <div className="flex justify-between items-center">
                             <span className="text-xs font-medium text-slate-500 dark:text-slate-400">
                                Min. Investment: <span className="text-slate-800 dark:text-white font-bold">₦{market.minInvestment.toLocaleString()}</span>
                             </span>
                             <button className="flex items-center gap-1 text-xs font-bold text-primary hover:text-primary/80 transition-colors">
                                View Opportunities <Icon name="arrow_forward" className="text-sm" />
                             </button>
                         </div>
                    </div>
                </div>
